...
const CategoryFormModal: React.FC<{
  ...
  isLoading?: boolean;
}> = ({ isOpen, onClose, onSave, category, isLoading = false }) => {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                placeholder="VD: Áo Thun, Quần Jean, Giày Dép..."
                required
              />
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              ...
              <div className="flex items-center space-x-3">
                <div className="w-16 h-16 bg-green-100 rounded-xl border border-green-200 flex items-center justify-center">
                  <Folder className="w-8 h-8 text-green-600" />
                </div>
                <div>
                  <p className="font-semibold text-gray-900">
                    {formData.ten_danh_muc || "Tên danh mục"}
                  </p>
            ...
            <div className="flex space-x-3 pt-4">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="flex-1 bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-xl font-semibold transition-colors flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading ? (
...
// Category Card Component cho Grid View
...
}> = ({ category, index, onView, onEdit, onDelete }) => {
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 overflow-hidden group flex flex-col">
      <div className="p-6 flex flex-col flex-grow">
        ...
        <p className="text-xs text-gray-400 mb-4">
          Tạo: {new Date(category.ngay_tao).toLocaleDateString("vi-VN")}
        </p>

        <div className="mt-auto pt-3 border-t border-gray-100">
          <div className="flex justify-center space-x-2">
            <button
              onClick={() => onView(category.ma_danh_muc)}
            ...
            <button
              onClick={() => onEdit(category.ma_danh_muc)}
            ...
            <button
              onClick={() => onDelete(category.ma_danh_muc)}
...
const ITEMS_PER_PAGE = 12; // Số danh mục mỗi trang

const QLDoanhMuc: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        ...
        setError((err as Error).message || "Không thể tải dữ liệu danh mục.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const allFilteredCategories = categories.filter((category) => {
    const searchLower = searchTerm.toLowerCase();
    return (
      category.ten_danh_muc.toLowerCase().includes(searchLower) ||
      (category.mo_ta && category.mo_ta.toLowerCase().includes(searchLower))
    );
  });
  ...
  const displayedCategories = allFilteredCategories.slice(
  ...
  const handleEditCategory = (id: number) => {
    const categoryToEdit = categories.find(
      (category) => category.ma_danh_muc === id
    );
    if (categoryToEdit) {
  ...
  const handleViewCategory = (id: number) => navigate(`/admin/danhmuc/${id}`);
  ...
  const handleSaveCategory = async (categoryData: Partial<Category>) => {
    ...
    try {
      if (editingCategory) {
        // Cập nhật danh mục
        const credentials: UpdateCategoryCredentials = {
          ten_danh_muc: categoryData.ten_danh_muc || "",
          mo_ta: categoryData.mo_ta || "",
        };
        const response = await categoryApi.update(
          editingCategory.ma_danh_muc,
          credentials
        );
        setCategories((prev) =>
          prev.map((cat) =>
            cat.ma_danh_muc === editingCategory.ma_danh_muc
              ? response.data
              : cat
          )
        );
        ...
        // Thêm danh mục mới
        const credentials: CreateCategoryCredentials = {
          ten_danh_muc: categoryData.ten_danh_muc || "",
          mo_ta: categoryData.mo_ta || "",
        };
      ...
      setError((err as Error).message || "Không thể lưu danh mục.");
    } finally {
  ...
  const handleDeleteCategory = async (categoryId: number) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa danh mục này?")) {
      try {
        ...
        setCategories((prev) =>
          prev.filter((category) => category.ma_danh_muc !== categoryId)
        );
      } catch (err) {
        setError((err as Error).message || "Không thể xóa danh mục.");
      }
    }
  };
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                          <div className="flex items-center space-x-3">
                            ...
                            <button
                              onClick={() =>
                                handleViewCategory(category.ma_danh_muc)
                              }
                        ...
                        <td className="px-6 py-4 text-center">
                          <div className="flex justify-center items-center space-x-1">
                            <button
                              ...
                              className="text-gray-400 hover:text-green-600 p-2 hover:bg-gray-100 rounded-lg"
                              title="Xem"
                            >
                              <Eye size={18} />
                            </button>
                            <button
                              onClick={() =>
                                handleEditCategory(category.ma_danh_muc)
                              }
                            ...
                            <button
                              onClick={() =>
                                handleDeleteCategory(category.ma_danh_muc)
                              }
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between p-4 border-t border-gray-200 mt-6">
                <span className="text-sm text-gray-700">
                  ...
                  trên tổng số {allFilteredCategories.length} danh mục
                </span>
