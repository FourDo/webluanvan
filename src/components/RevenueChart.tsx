...
const RevenueChart: React.FC<RevenueChartProps> = ({ data, title, type }) => {
  ...
  const maxRevenue = Math.max(...data.map((item) => item.doanh_thu));
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6">
      ...
      <div className="space-y-4">
        {data.map((item, index) => {
          const percentage =
            maxRevenue > 0 ? (item.doanh_thu / maxRevenue) * 100 : 0;
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {formatNumber(item.don_hang)} đơn hàng
                    </p>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  {item.tang_truong !== undefined && (
                    <div className="flex items-center justify-end space-x-1">
                      ...
                      >
                        {Math.abs(item.tang_truong).toFixed(1)}%
                      </span>
                    </div>
                  )}
                </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-purple-600">
              {Math.round(
                data.reduce((sum, item) => sum + item.don_hang, 0) / data.length
              )}
            </p>
          </div>
        </div>
